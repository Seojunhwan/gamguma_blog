...
export const Footer = () => {
  return (
    <footer className='z-10 mt-12 w-full'>
      <div className='mx-auto flex w-full max-w-5xl flex-col items-center justify-between border-t-[1px] px-4 pb-14 pt-12 sm:flex-row lg:px-0'>
        ...
        <ul className='mt-4 flex space-x-6'>
          {Object.entries(config.social).map(
            ([social, link]) =>
              link && (
                <li key={social}>
                  <Link href={(social === 'email' ? 'mailto:' : '') + link}>
                    <IconButton
